...
function SecondarySwipeButton({ children, size, asChild = false, className, ...props }: SwipeButtonProps) {
  return (
    <Button
      ...
      asChild={asChild}
      className={cn(
        'ring-primary/60 hover:bg-primary/10 text-primary bg-primary/10 relative overflow-hidden rounded-full shadow-[inset_0_-3px_6px_0px_rgba(30,184,206,0.30)] ring-2 duration-500 hover:shadow-[inset_0_-3px_6px_-2px_rgba(30,184,206,0.10)]',
        size === 'lg' && 'text-base has-[>svg]:px-6',
        className
      )}
      {...props}
